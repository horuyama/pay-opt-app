...
function canon(s){
  // remove hyphens (ASCII and Unicode), middle dot, spaces
  return norm(s).replace(/[\u2010\u2011\u2012\u2013\u2014\u2212\-・\s]/g,'')
                .replace(/ﾌｧﾐﾘｰﾏｰﾄ/g,'ファミリーマート')
                .replace(/ｾﾌﾞﾝｲﾚﾌﾞﾝ/g,'セブンイレブン')
                .replace(/ﾛｰｿﾝ/g,'ローソン');
}
...
async function loadData(){
  campaigns = await fetch('campaigns.json').then(r=>r.json());
  cards = await fetch('cards.json').then(r=>r.json());
}
function inferCategory(tags){
  const hay = (tags.shop||'')+' '+(tags.amenity||'')+' '+(tags.brand||'')+' '+(tags.name||'');
  const t = hay.toLowerCase();
  if(t.includes('convenience')||t.includes('seven')||t.includes('7-eleven')||hay.includes('セブン')||hay.includes('セブンイレブン')||hay.includes('ファミリーマート')||hay.includes('ﾌｧﾐﾘｰﾏｰﾄ')||hay.includes('ローソン')) return 'convenience_store';
...
function matchCampaigns(name, category){
  ...
  return campaigns.filter(c=>{
    ...
    const storeOk = !c.match ? (!!c.store && like(nm,c.store)) : (c.match||[]).some(m=>like(nm,m));
    const catOk = !c.category || c.category===category || storeOk; // storeが合えばカテゴリは緩和
    return okTime && catOk && (storeOk || !c.match && !c.store);
  });
}
function summarizeRate(name, category){
  ...
  const list = cards.map(card=>{
    ...
    hits.forEach(h=>{
      ...
      } else if(h.brand && h.brand===card.name){
        // old schema support
        bonus += (h.bonus||0);
        why.push(`${h.name}+${h.bonus||0}%`);
      }
    });
...
async function scan(){
  ...
  if(!data){ toast('周辺検索に失敗しました（混雑中）。少し待って再試行してください'); return; }
  const j = data;
  const markers = [];
  ...
  (j.elements||[]).forEach(el=>{
    ...
    const {list} = summarizeRate(name, cat);
    const top = list[0];
    // style: campaign hit if any card got > base
    const hasBoost = list.some(x=>x.why.length>0);
    const icon = L.divIcon({className:'', html:`<div style="transform:translate(-50%,-100%);">
      <svg width="24" height="32" viewBox="0 0 24 32">
        <path d="M12 0C6 0 1.5 4.5 1.5 10.5c0 7.5 10.5 21 10.5 21s10.5-13.5 10.5-21C22.5 4.5 18 0 12 0z" fill="${hasBoost?'#e74c3c':'#bdc3c7'}" stroke="#ffffff" stroke-width="1"/>
